...
ArrayList<Integer> runAstar(Vec2[] nodePos, int numNodes, Vec2 startPos, Vec2 goalPos, Vec2[] centers, float[] radii){
  ...
  HashSet<Integer> fringe = new HashSet();  //New empty fringe
  ArrayList<Integer> path = new ArrayList();
  ...
  for (int i = 0; i < numNodes; i++) { //Clear visit tags and parent pointers
    ...
    if (!circleListCheck.hit){
      visited[i] = true;
      fringe.add(i);
      gScore.put(i, distBetween);
      fScore.put(i, distBetween + nodePos[i].distanceTo(goalPos));
    }  
  }
  ...
  while (fringe.size() > 0){
    ...
    int currentNode = 0;
    for (int item : fringe) {
      float f = fScore.getOrDefault(item, Float.MAX_VALUE);
      if (f < minValue) {
        currentNode = item;
        minValue = f;
      }
    ...
    for (int i = 0; i < neighbors[currentNode].size(); i++){
      int neighborNode = neighbors[currentNode].get(i);
      float tentative_gScore = gScore.getOrDefault(currentNode, Float.MAX_VALUE) + nodePos[currentNode].distanceTo(nodePos[neighborNode]);
      if (tentative_gScore < gScore.getOrDefault(neighborNode, Float.MAX_VALUE)) {
        gScore.put(neighborNode, tentative_gScore);
        fScore.put(neighborNode, tentative_gScore + nodePos[currentNode].distanceTo(goalPos));
